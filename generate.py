...
def render_document(doc, gitrev):
    ...
    with open(f"emonTH.kicad_{doc}", 'r') as f_in:
        with open(f"{outdir}/emonTH.kicad_{doc}", 'w') as f_out:
            found_gitrev = False
            for ln in f_in:
                if not found_gitrev:
...
if __name__ == "__main__":
    ...
    print("> Generating emonTH3 manufacturing files...")
    ...
    sch_pdf_cmd = f"{kicad_cli} sch export pdf -o emonTH3-schematic.pdf emonTH.kicad_sch"
    ...
    pcb_3d_cmd = f"{kicad_cli} pcb export step -f -o {outdir}/emonTH3-3d-render.step {outdir}/emonTH.kicad_pcb"
    ...
    bom_xml_cmd = f"{kicad_cli} sch export python-bom -o {outdir}/bom-tmp.xml {outdir}/emonTH.kicad_sch"
    bom_csv_cmd = f"python3 {kicad_script}/bom_csv_grouped_by_value_with_fp.py {outdir}/bom-tmp.xml {outdir}/emonTH3-bom.csv"
    ...
    with zipfile.ZipFile(f"{outdir}/emonTH3-gerbers.zip", 'w') as z:
